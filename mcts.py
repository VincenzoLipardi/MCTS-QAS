...
class Node:
    ...
    def __repr__(self):
        return "Tree depth: %s  -  Generated by Action: %s  -  Number of Children: %s \nVisits: %s  -  Value: %s \nQuantum Circuit (CNOTS counts= %s):\n%s" % \
    ...
    def define_children(self, prob_choice, stop_deterministic=False, roll_out=False):
...
def commit(epsilon, budget, current_node):
    # It commits to the best action if mcts explored already enough
    if epsilon is not None:
        coin = random.uniform(0, 1)

        while coin < epsilon and current_node.visits > 100:
...
def mcts(root, budget, evaluation_function, rollout_type, roll_out_steps, branches, choices, epsilon, stop_deterministic, verbose=False):
    ...
    for _ in range(budget):
        current_node = root
        if epoch_counter > budget/10:
            commit(epsilon, budget, current_node)
        ...
        if verbose:
            print('Reward: ', result)
            print('Backpropagation starts from node:\n', current_node)
        # Backpropagation
